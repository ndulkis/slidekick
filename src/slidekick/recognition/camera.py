...
from collections import deque   #Stores recent frame history for tracking dynamic gesture displacement
import time #Used to determine cooldown/debounce
from events import create_gesture_event, create_landmark_event #Creates standardized SlideKick gesture and landmark event payloads
...
MAX_SWIPE_DURATION = 0.50   #Helps differentiate between intentional swipes and repositioning
...
def detect_swipe(hand_landmarks, position_history, last_swipe_time, current_time):
    ...
    if len(position_history) > 1:
        ...
        right_movements = sum(
            movement > 0 for movement in frame_movements
        )
...
def get_landmark_points(hand_landmarks, frame):
    ...
    frame_height, frame_width, _ = frame.shape  #floating "_" is ignoring the color channel value which we dont need
...
def process_hand_detection(
    ...
    if result.hand_landmarks:
        ...
        landmark_event = create_landmark_event(
            hand_landmarks,
            current_time
        )
...
def should_close_window(window_name):
    ...
    # Detect if the user clicked the X button.
